...
const NotificationList = () => {
  ...
  const handleNavigateToPost = ( id : string) => {
    router.push(`/(posts)/PostDetail?id=${id}`);
  }
  ...
  const renderItem = ({ item }: { item: NotificationModel }) => (
    <Card
      size="md"
      variant="filled"
      className={`m-3 ${!item.view ? "bg-info-0" : "bg-secondary-0"}`}
    >
      <Pressable className="" onPress={() => handleNavigateToPost(item.notification.post.id)}>
        {({ pressed }) => (
          <Box className={pressed ? "opacity-75" : ""}>
            ...
            <VStack space="xs">
              ...
              <Text className="text-info-600" size="sm">
