...
const Transaction = () => {
  ...
  const validateAmount = (value: string) => {
    ...
    if (isNaN(num) || num < minAmount) {
      setError('Số tiền phải lớn hơn ' + minAmount.toLocaleString() + ' VND');
    } else if (user?.balance && num > user?.balance && type === 'withdraw') {
      setError('Số tiền rút phải nhỏ hơn số dư');
    } else {
      isValid = true;
      setError('');
    }
          ...
          render: ({ id }) => {
            ...
            return (
              <Toast nativeID={uniqueToastId} action="error" variant="outline">
                <ToastTitle>Nạp tiền thất bại</ToastTitle>
      ...
      if (withdrawError || res.data?.returnCode !== 1000) {
        toast.show({
          ...
          render: ({ id }) => {
            ...
            return (
              <Toast nativeID={uniqueToastId} action="error" variant="outline">
                <ToastTitle>Rút tiền thất bại</ToastTitle>
                <ToastDescription>{res.error.data.message}</ToastDescription>
              </Toast>
            );
          },
          ...
          render: ({ id }) => {
            ...
            return (
              ...
              >
                ...
                <ToastDescription>Rút tiền thành công</ToastDescription>
              </Toast>
            );
          },
        });
    ...
    <SafeAreaView className="flex-1 h-full">
        ...
        <FormControl isInvalid={error != ''} size="md">
          <Box className="rounded-lg p-3 bg-success-300 mb-2">
            <View className="p-2 rounded-lg my-2 flex-row justify-between items-center bg-success-400">
              <Text className="text-white text-lg">Số dư hiện tại :</Text>
      ...
      <Box className="sticky bottom-0 p-4 w-full">
        ...
        >
          ...
          <ButtonText>{type === 'withdraw' ? 'Rút' : 'Nạp'} tiền</ButtonText>
        </Button>
      </Box>
    </SafeAreaView>
  );
